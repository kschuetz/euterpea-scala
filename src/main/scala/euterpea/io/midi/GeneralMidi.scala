...
import euterpea.music.note.Music.InstrumentName._
...
object GeneralMidi {
  ...
  def toGM(iName: InstrumentName): Int = iName match {
    case Percussion => 0
    case Custom(_) => 0
    case i => fromEnum(i)
  }

  implicit val enumInstrumentName = new Enum[InstrumentName] {
    override def toEnum(i: Int): InstrumentName = i match {
    ...
    override def fromEnum(a: InstrumentName): Int = a match {
